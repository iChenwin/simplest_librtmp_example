...
#define HTONTIME(x) ((x>>16&0xff)|(x<<16&0xff0000)|(x&0xff00)|(x&0xff000000))			//exchange the second byte and the forth byte
...
int publish_using_packet(){
	...
	rtmp->Link.timeout=5;			
	if(!RTMP_SetupURL(rtmp, (char *)"rtmp://localhost:1935/rtmplive/live"))
	{
	...
	//if unable,the AMF command would be 'play'(receive) instead of 'publish'(send)
